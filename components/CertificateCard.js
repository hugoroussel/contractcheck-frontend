...
const CertificateCard = (certificate) => {
    ...
    async function getNumberOfValidators(){
        let validators = await getAllValidatorsOfContract(certificate.certificate[0])
        setTotalValidators(validators.length)
    }

    return(
        <li key={certificate.certificate[1]} className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow">
        <div className="flex w-full items-center justify-between space-x-6 p-6">
          <div className="flex-1 truncate">
            <div className="flex items-center space-x-3">
              <h3 className="truncate text-sm font-medium text-gray-900">{certificate.certificate[1]}</h3>
            </div>
            <p className="mt-1 truncate text-sm text-gray-500">Certificate creation : {unixTimeStamptoDate(certificate.certificate[6])}</p>
            <p className="mt-1 truncate text-sm text-gray-500">Network : {chainIdToNetworkName(certificate.certificate[4])}</p>
          </div>
          <img className="h-10 w-10 flex-shrink-0 rounded-full bg-gray-300" src={chainIdToNetworkIcon(certificate.certificate[4])} alt="" />
        </div>
        <div>
          <div className="-mt-px flex divide-x divide-gray-200">
            <div className="flex w-0 flex-1">
              <a
                ...
                href={`/certificate/`+certificate.certificate[0]}
              >
            ...
            <div className="-ml-px flex w-0 flex-1">
              ...
              >
                ...
                <span className="ml-3">
                  <a href={"/validators/"+certificate.certificate[0]}>
